 ...
 * Might improve compatibility with direct input but will report a 2 axis ffb
 * compatible device
 */
...
#define TIM_MICROS_HALTICK                                                     \
  htim7 // Micros timer MUST be reset by hal tick timer or isr to count
        // microseconds since last tick
#define TIM_USER                                                               \
  htim9 // Timer with full core clock speed available for the mainclass
...
#define ADC_CHAN_FPIN                                                          \
  0 // First analog channel pin. last channel = fpin+ADC_PINS-1
...
#define EEPROM_START_ADDRESS                                                   \
  ((uint32_t)0x08004000) /* EEPROM emulation start address: from sector1*/
